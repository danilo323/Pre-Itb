...
El contenido del sitio web (textos, imágenes, programas, solicitudes de admisión y ajustes del panel) vive exclusivamente en la **Base de Datos MySQL** (tablas `site_content` y `form_registros`).

Cuando levantas el proyecto por primera vez usando Docker, la base de datos se crea y se llena automáticamente usando el archivo `database/init.sql`. Cualquier cambio que hagas desde el panel de administración se guardará en tu base de datos local.

## 🚀 Cómo correr el proyecto

Hay tres formas de levantarlo — usa la que te resulte más cómoda, las tres funcionan igual:
...
3. Entra por `http://localhost/itb` (o el nombre de carpeta que hayas usado) en vez de usar los puertos `8080` o `8000`.
...
### Opción C: Docker Compose (con Base de Datos)
...
> ⚠️ **Un detalle muy importante para ver los cambios en tu Docker:**
>
> Cuando corres `docker compose up -d`, Docker guarda los datos en tu disco para que no se te borren al apagar la PC. Por eso, si tu compañero subió cambios a `database/init.sql`, para que tu Docker vuelva a leer ese archivo con las novedades de tu compañero, solo ejecutas:
>
> ```bash
...
> ```
>
> (El `-v` le dice a Docker: "limpia mi base de datos vieja y vuelve a cargar el `init.sql` nuevo que bajé de GitHub").
> En 5 segundos tu página web tendrá exactamente los mismos textos e imágenes que subió tu compañero.

> 🛑 **Cómo apagar el servidor (para el día a día):**
> 
> Cuando termines de trabajar y quieras apagar el servidor **sin perder los cambios locales de tu base de datos**, usa:
> ```bash
> docker compose stop
> ```
> *(Para volver a prenderlo al día siguiente, usa `docker compose start` o `docker compose up -d`)*.

### Paso 2: Abrir las páginas en tu navegador

Abre Chrome, Firefox o cualquier navegador y entra a las siguientes direcciones, dependiendo de qué opción elegiste para levantar el servidor:

#### 💻 Si usaste la Terminal con PHP (Opción A)
...
- ⚙️ **Ejemplo de singleton**: [http://localhost:8000/admin/singleton.php?c=ajustes](http://localhost:8000/admin/singleton.php?c=ajustes)

#### 🐘 Si usaste XAMPP / Apache (Opción B)
- 🌐 Entra por `http://localhost/itb` (o el nombre de la carpeta donde guardaste el proyecto). Las rutas del panel serían `http://localhost/itb/admin` y así sucesivamente.

#### 🐳 Si usaste Docker (Opción C)
- 🌐 **Landing Page**: [http://localhost:8080](http://localhost:8080)
- 🔐 **Panel Admin**: [http://localhost:8080/admin](http://localhost:8080/admin)
- 🔑 **Login**: [http://localhost:8080/admin/login](http://localhost:8080/admin/login) — (si ya tienes sesión abierta, te redirige solo al panel).
- 🗄️ **Base de Datos (phpMyAdmin)**: [http://localhost:8081](http://localhost:8081)
- ⚙️ **Ejemplo de singleton**: [http://localhost:8080/admin/singleton.php?c=ajustes](http://localhost:8080/admin/singleton.php?c=ajustes)

> En un hosting real (Apache) o con XAMPP, no hace falta el `router.php`: el `.htaccess` de la raíz ya define las mismas rutas limpias.
