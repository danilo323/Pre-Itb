...
// Lee los datos guardados en la base de datos MySQL (persistente),
// con fallback a la sesión y finalmente el 'default' del schema
function get_saved_data($sec_key, $field_key, $default) {
    $val = storage_get($sec_key, $field_key, '');
    if ($val !== '') {
        ...
        <div class="order-block">
            ...
            <div class="order-list">
                ...
                foreach ($config['sections'] as $sub_key => $sub_config): 
                    $vis_val = storage_get($sub_key, '_visible', '');
                    if ($vis_val === '') {
                        $vis_val = $_SESSION['admin_data'][$sub_key]['_visible'] ?? '1';
                    }
                    $is_visible = ($vis_val === '1');
                ?>
